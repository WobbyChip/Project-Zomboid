    ...
    public void parse(final ScriptParser.Block block) {
        for (final ScriptParser.Value value : block.values) {
            ...
            if ("source".equalsIgnoreCase(trim)) {
                ...
                if (!"sourceFrame".equalsIgnoreCase(trim)) {
                    continue;
                }
                this.m_sourceFrame = PZMath.tryParseInt(trim2, 1);
            }
        }
    }
    ...
    public void exec(final List<Keyframe> list) {
        ...
        for (int i = 0; i < 60; ++i) {
            ...
            if (boneFrames.length != 0) {
                ...
                keyframe2.Time = (this.m_frame - 1) / (float)this.m_FPS;
                keyframe2.Position = KeyframeUtil.GetKeyFramePosition(boneFrames, (this.m_sourceFrame - 1) / (float)this.m_sourceFPS, animationClip.Duration);
                keyframe2.Rotation = KeyframeUtil.GetKeyFrameRotation(boneFrames, (this.m_sourceFrame - 1) / (float)this.m_sourceFPS, animationClip.Duration);
