...
function ISHotwireVehicle:start()
	self.sound = self.character:playSound("VehicleHotwireStart")
end
...
function ISHotwireVehicle:stopSound()
	if self.sound and self.character:getEmitter():isPlaying(self.sound) then
		if self.character:getEmitter():hasSustainPoints(self.sound) then
			self.character:getEmitter():triggerCue(self.sound)
		else
