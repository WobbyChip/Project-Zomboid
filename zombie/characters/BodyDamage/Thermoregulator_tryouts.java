...
public class Thermoregulator_tryouts
{
    ...
    public Thermoregulator_tryouts(final BodyDamage bodyDamage) {
        this.setPoint = 37.0f;
    ...
    public static void setSimulationMultiplier(final float simulation_MULTIPLIER) {
        Thermoregulator_tryouts.SIMULATION_MULTIPLIER = simulation_MULTIPLIER;
    }
    ...
    private void initNodes() {
        ...
        for (int i = 0; i < this.bodyDamage.getBodyParts().size(); ++i) {
            ...
            ThermalNode core = null;
            switch (bodyPart.getType()) {
                case Torso_Upper: {
                    core = new ThermalNode(true, 37.0f, bodyPart, 0.25f);
                    this.core = core;
                    break;
                }
                case Head: {
                    core = new ThermalNode(37.0f, bodyPart, 1.0f);
                    break;
                }
                case Neck: {
                    core = new ThermalNode(37.0f, bodyPart, 0.5f);
                    break;
                }
                case Torso_Lower: {
                    core = new ThermalNode(37.0f, bodyPart, 0.25f);
                    break;
                }
                case Groin: {
                    core = new ThermalNode(37.0f, bodyPart, 0.5f);
        ...
        for (int j = 0; j < this.nodes.length; ++j) {
            final ThermalNode thermalNode = this.nodes[j];
            final BodyPartType parent = BodyPartContacts.getParent(thermalNode.bodyPartType);
            if (parent != null) {
                thermalNode.upstream = this.getNodeForType(parent);
            }
            final BodyPartType[] children = BodyPartContacts.getChildren(thermalNode.bodyPartType);
            if (children != null && children.length > 0) {
                thermalNode.downstream = new ThermalNode[children.length];
                for (int k = 0; k < children.length; ++k) {
                    thermalNode.downstream[k] = this.getNodeForType(children[k]);
                }
            }
        }
    ...
    public float getSimulationMultiplier() {
        return Thermoregulator_tryouts.SIMULATION_MULTIPLIER;
    }
    ...
    private float getSimulationMultiplier(final Multiplier multiplier) {
        ...
        return multiplier2 * Thermoregulator_tryouts.SIMULATION_MULTIPLIER;
    }
    ...
    private void updateTest() {
        ...
        for (int i = 0; i < this.nodes.length; ++i) {
            ...
            thermalNode.heatDelta = airTemperature - thermalNode.skinCelcius;
            final ThermalNode thermalNode2 = thermalNode;
            thermalNode2.heatDelta /= 1.0f + thermalNode.insulation;
            final ThermalNode thermalNode3 = thermalNode;
            thermalNode3.heatDelta /= 3.75f;
            final float n = this.metabolicRateReal * (1.0f + thermalNode.insulation * 0.25f);
            final ThermalNode thermalNode4 = thermalNode;
            thermalNode4.heatDelta += n;
            coreHeatDelta += thermalNode.heatDelta * thermalNode.skinSurface * 1.0f;
            float airTemperature2 = this.core.celcius - 20.0f;
            final float celcius = this.core.celcius;
            if (airTemperature2 < this.airTemperature) {
                airTemperature2 = this.airTemperature;
            }
            if (airTemperature2 > celcius) {
                airTemperature2 = celcius;
            }
            final float n2 = PZMath.clamp(thermalNode.skinCelcius + thermalNode.heatDelta, airTemperature2, celcius) - thermalNode.skinCelcius;
            float n3 = 0.0025f;
            if ((thermalNode.skinCelcius < 33.0f && n2 < 0.0f) || (thermalNode.skinCelcius > 33.0f && n2 > 0.0f)) {
                n3 = 2.5E-4f;
            }
            final ThermalNode thermalNode5 = thermalNode;
            thermalNode5.skinCelcius += n2 * n3;
        }
        this.coreHeatDelta = coreHeatDelta;
        float n4 = 0.0025f;
        ...
        core.celcius += this.coreHeatDelta * n4;
    }
    ...
    private void updateTest_4() {
        ...
        for (int i = 0; i < this.nodes.length; ++i) {
            final ThermalNode thermalNode = this.nodes[i];
            thermalNode.calculateInsulation();
            final float n2 = 1.0f - thermalNode.getDistToCore();
            float airTemperature = this.airTemperature;
            if (this.airAndWindTemp < this.airTemperature) {
                airTemperature -= (this.airTemperature - this.airAndWindTemp) / (1.0f + thermalNode.windresist);
            }
            ...
            thermalNode2.heatDelta /= 1.0f + thermalNode.insulation;
            final ThermalNode thermalNode3 = thermalNode;
            thermalNode3.heatDelta /= 12.0f;
            n += thermalNode.heatDelta * thermalNode.skinSurface * 1.0f;
            float airTemperature2 = this.core.celcius - 20.0f;
            final float celcius = this.core.celcius;
            if (airTemperature2 < this.airTemperature) {
                airTemperature2 = this.airTemperature;
            ...
            final float n3 = this.core.celcius - this.setPoint;
            float n4;
            if (n3 > 0.0f) {
                n4 = n3 * 2.0f * (1.0f + n2 * 0.25f);
            }
            else {
                n4 = n3 * (1.0f + thermalNode.distToCore * 0.25f);
            }
            final float n5 = PZMath.clamp(thermalNode.skinCelcius + thermalNode.heatDelta + n4, airTemperature2, celcius) - thermalNode.skinCelcius;
            float n6 = 0.0025f;
            if ((thermalNode.skinCelcius < 33.0f && n5 < 0.0f) || (thermalNode.skinCelcius > 33.0f && n5 > 0.0f)) {
                n6 = 2.5E-4f;
            }
            final ThermalNode thermalNode4 = thermalNode;
            thermalNode4.skinCelcius += n5 * n6;
        }
        float n7 = this.metabolicRateReal / Metabolics.Default.getMet() - 1.0f;
        if (n7 > 0.0f) {
            n7 /= Metabolics.Default.getMet() * 2.0f;
        }
        this.coreHeatDelta = n + n7;
        float n8 = 0.0025f;
        if (this.core.celcius < this.setPoint && this.coreHeatDelta < 0.0f) {
            n8 = 5.0E-5f;
        }
        ...
        final ThermalNode core = this.core;
        core.celcius += this.coreHeatDelta * n8;
    }
    ...
    private void updateTest_3() {
        for (int i = 0; i < this.nodes.length; ++i) {
            ...
            final ThermalNode thermalNode2 = thermalNode;
            thermalNode2.heatDelta /= 1.0f + thermalNode.insulation;
            final float n = this.metabolicRateReal * (1.0f + (1.0f - thermalNode.getDistToCore() * 0.5f)) * (1.0f + thermalNode.insulation * 0.25f);
            final ThermalNode thermalNode3 = thermalNode;
            thermalNode3.heatDelta += n;
            float airTemperature2 = this.core.celcius - 20.0f;
            final float celcius = this.core.celcius;
            if (airTemperature2 < this.airTemperature) {
                airTemperature2 = this.airTemperature;
            ...
            final float n2 = PZMath.clamp(thermalNode.skinCelcius + thermalNode.heatDelta / 12.0f, airTemperature2, celcius) - thermalNode.skinCelcius;
            final ThermalNode thermalNode4 = thermalNode;
            thermalNode4.skinCelcius += n2 * this.getSimulationMultiplier(Multiplier.SkinCelcius);
        }
    }
    ...
    private void updateTest_2() {
        for (int i = 0; i < this.nodes.length; ++i) {
            ...
            thermalNode.heatDelta = airTemperature - 27.0f;
            final ThermalNode thermalNode2 = thermalNode;
            thermalNode2.heatDelta /= 1.0f + thermalNode.insulation;
            final float n = this.metabolicRateReal * (1.0f + (1.0f - thermalNode.getDistToCore() * 0.8f)) * (1.0f + thermalNode.insulation);
            final ThermalNode thermalNode3 = thermalNode;
            thermalNode3.heatDelta += n;
            final float n2 = PZMath.clamp(thermalNode.heatDelta / 12.0f, -1.0f, 1.0f) - thermalNode.primaryDelta;
            final ThermalNode thermalNode4 = thermalNode;
            thermalNode4.primaryDelta += n2 * this.getSimulationMultiplier(Multiplier.PrimaryDelta);
            thermalNode.primaryDelta = PZMath.clamp(thermalNode.primaryDelta, -1.0f, 1.0f);
        }
        for (int j = 0; j < this.nodes.length; ++j) {
            final ThermalNode thermalNode5 = this.nodes[j];
            float airTemperature2 = this.core.celcius - 20.0f;
            final float celcius = this.core.celcius;
            if (airTemperature2 < this.airTemperature) {
                airTemperature2 = this.airTemperature;
            }
            final float n3 = this.core.celcius - 4.0f;
            float n4;
            if (thermalNode5.primaryDelta < 0.0f) {
                n4 = 33.0f - 20.0f * PZMath.abs(thermalNode5.primaryDelta);
            }
            else {
                n4 = 33.0f + 20.0f * thermalNode5.primaryDelta;
            }
            final float clamp = PZMath.clamp(n4, airTemperature2, celcius);
            final float n5 = clamp - thermalNode5.skinCelcius;
            final ThermalNode thermalNode6 = thermalNode5;
            thermalNode6.skinCelcius += n5 * this.getSimulationMultiplier(Multiplier.SkinCelcius);
            thermalNode5.skinCelcius = clamp;
        }
    }
    ...
    private void updateTest_OLD() {
        ...
        for (int j = 0; j < this.nodes.length; ++j) {
            final ThermalNode thermalNode4 = this.nodes[j];
            float airTemperature2 = this.core.celcius - 20.0f;
            final float celcius = this.core.celcius;
            if (airTemperature2 < this.airTemperature) {
                airTemperature2 = this.airTemperature;
            }
            final float n2 = this.core.celcius - 4.0f;
            float n3;
            if (thermalNode4.primaryDelta < 0.0f) {
                n3 = 33.0f - 20.0f * PZMath.abs(thermalNode4.primaryDelta);
            }
            else {
                n3 = 33.0f + 20.0f * thermalNode4.primaryDelta;
            }
            final float clamp = PZMath.clamp(n3, airTemperature2, celcius);
            final float n4 = clamp - thermalNode4.skinCelcius;
            final ThermalNode thermalNode5 = thermalNode4;
            thermalNode5.skinCelcius += n4 * this.getSimulationMultiplier(Multiplier.SkinCelcius);
            thermalNode4.skinCelcius = clamp;
        }
    }
    ...
    private void updateNodesHeatDelta() {
        ...
        for (int i = 0; i < this.nodes.length; ++i) {
            ...
            if (thermalNode.primaryDelta > 0.0f) {
                final float n7 = Metabolics.Default.getMet() * thermalNode.primaryDelta * thermalNode.skinSurface / thermalNode.insulation * ((0.2f + 0.8f * this.getBodyFluids()) * (0.1f + 0.9f * n2)) * n3 * (1.0f - 0.2f * n) * (1.0f + 0.2f * fitness);
                final ThermalNode thermalNode2 = thermalNode;
            ...
            if (thermalNode.secondaryDelta > 0.0f) {
                final float n9 = Metabolics.MAX.getMet() * 0.75f * thermalNode.secondaryDelta * thermalNode.skinSurface / thermalNode.insulation * (0.1f + 0.9f * this.getBodyFluids()) * (0.85f + 0.15f * n3) * (1.0f - 0.2f * n) * (1.0f + 0.2f * fitness);
                final ThermalNode thermalNode4 = thermalNode;
    ...
    private void updateNodesHeatDelta_OLD() {
        ...
        float n4 = 1.0f;
        if (this.core.celcius < 37.0f) {
            final float n5 = (this.core.celcius - 20.0f) / 17.0f;
            n4 = n5 * n5;
        }
        float totalHeatRaw = 0.0f;
        for (int i = 0; i < this.nodes.length; ++i) {
            ...
            final float n6 = 1.0f + thermalNode.insulation;
            float n7 = airTemperature - thermalNode.skinCelcius;
            if (n7 < 0.0f) {
                n7 *= 1.0f + 0.25f * thermalNode.bodyWetness;
            }
            else if (n7 > 0.0f) {
                n7 /= 1.0f + thermalNode.bodyWetness * 4.0f;
            }
            thermalNode.heatDelta = n7 * 0.3f / n6 * thermalNode.skinSurface;
            if (thermalNode.primaryDelta > 0.0f) {
                final float n8 = Metabolics.Default.getMet() * 1.0f * thermalNode.primaryDelta * thermalNode.skinSurface / n6 * ((0.2f + 0.8f * this.getBodyFluids()) * (0.1f + 0.9f * n2)) * n3 * (1.0f - 0.2f * n) * (1.0f + 0.2f * fitness);
                final ThermalNode thermalNode2 = thermalNode;
                thermalNode2.heatDelta -= n8;
            }
            else {
                final float n9 = Metabolics.Default.getMet() * 1.0f * PZMath.abs(thermalNode.primaryDelta) * thermalNode.skinSurface * (0.2f + 0.8f * this.getEnergy()) * (1.0f + 0.2f * n) * (1.0f + 0.2f * fitness) * (0.25f + 0.75f * n4);
                final ThermalNode thermalNode3 = thermalNode;
                thermalNode3.heatDelta += n9;
            }
            if (thermalNode.secondaryDelta > 0.0f) {
                final float n10 = Metabolics.MAX.getMet() * 0.75f * thermalNode.secondaryDelta * thermalNode.skinSurface / n6 * (0.1f + 0.9f * this.getBodyFluids()) * (0.75f + 0.25f * n3) * (1.0f - 0.2f * n) * (1.0f + 0.2f * fitness);
                final ThermalNode thermalNode4 = thermalNode;
                thermalNode4.heatDelta -= n10;
            }
            else {
                final float n11 = Metabolics.Default.getMet() * 1.0f * PZMath.abs(thermalNode.secondaryDelta) * thermalNode.skinSurface * (0.1f + 0.9f * this.getEnergy()) * (1.0f + 0.2f * n) * (1.0f + 0.2f * fitness) * (0.25f + 0.75f * n4);
                final ThermalNode thermalNode5 = thermalNode;
                thermalNode5.heatDelta += n11;
            }
            totalHeatRaw += thermalNode.heatDelta;
        }
        this.totalHeatRaw = totalHeatRaw;
    ...
    private void updateBodyMultipliers() {
        ...
        this.fluidsMultiplier = 1.0;
        this.fatigueMultiplier = 1.0;
        final float n = this.metabolicRateReal / Metabolics.Default.getMet();
        this.energyMultiplier = PZMath.clamp(n * n, 1.0f, 5.0f);
        final float abs = PZMath.abs(this.primTotal);
        final float n2 = abs * abs;
        if (this.primTotal < 0.0f) {
            this.energyMultiplier += 0.75f * n2;
            this.fatigueMultiplier += 0.5f * n2;
        }
        else if (this.primTotal > 0.0f) {
            this.fluidsMultiplier += 0.75f * n2;
            this.fatigueMultiplier += 0.5f * n2;
        }
        final float abs2 = PZMath.abs(this.secTotal);
        final float n3 = abs2 * abs2;
        if (this.secTotal < 0.0f) {
            this.energyMultiplier += 8.0f * n3;
            this.fatigueMultiplier += 3.5f * n3;
        }
        else if (this.secTotal > 0.0f) {
            this.fluidsMultiplier += 6.0f * n3;
            this.fatigueMultiplier += 3.5f * n3;
        }
    }
    
    private void updateClothing() {
        this.character.getItemVisuals(Thermoregulator_tryouts.itemVisuals);
        int n = (Thermoregulator_tryouts.itemVisuals.size() != Thermoregulator_tryouts.itemVisualsCache.size()) ? 1 : 0;
        if (n == 0) {
            for (int i = 0; i < Thermoregulator_tryouts.itemVisuals.size(); ++i) {
                if (i >= Thermoregulator_tryouts.itemVisualsCache.size() || Thermoregulator_tryouts.itemVisuals.get(i) != Thermoregulator_tryouts.itemVisualsCache.get(i)) {
                    n = 1;
                    break;
                }
            }
        ...
        if (n != 0) {
            ...
            Thermoregulator_tryouts.itemVisualsCache.clear();
            for (int k = 0; k < Thermoregulator_tryouts.itemVisuals.size(); ++k) {
                final ItemVisual e = Thermoregulator_tryouts.itemVisuals.get(k);
                final InventoryItem inventoryItem = e.getInventoryItem();
                Thermoregulator_tryouts.itemVisualsCache.add(e);
                if (inventoryItem instanceof Clothing) {
                    ...
                    if (clothing.getInsulation() > 0.0f || clothing.getWindresistance() > 0.0f) {
                        ...
                        if (bloodClothingType != null) {
                            Thermoregulator_tryouts.coveredParts.clear();
                            BloodClothingType.getCoveredParts(bloodClothingType, Thermoregulator_tryouts.coveredParts);
                            for (int l = 0; l < Thermoregulator_tryouts.coveredParts.size(); ++l) {
                                final BloodBodyPartType bloodBodyPartType = Thermoregulator_tryouts.coveredParts.get(l);
    ...
    public float getEnergy() {
        return 0.6f * (1.0f - this.stats.getHunger() * this.stats.getHunger()) + 0.4f * (1.0f - this.stats.getFatigue() * this.stats.getFatigue());
    }
    ...
    static {
        ...
        Thermoregulator_tryouts.SIMULATION_MULTIPLIER = 1.0f;
    }
    ...
    public class ThermalNode
    {
        ...
        public ThermalNode(final Thermoregulator_tryouts thermoregulator_tryouts, final float n, final BodyPart bodyPart, final float n2) {
            this(thermoregulator_tryouts, false, n, bodyPart, n2);
        }
