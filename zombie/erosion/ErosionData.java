...
public final class ErosionData
{
    ...
    public static final class Square
    {
        ...
        public final ArrayList<ErosionCategory.Data> regions;
        private static final int[] rands;
        
        public Square() {
            this.init = false;
            this.doNothing = false;
            this.regions = new ArrayList<ErosionCategory.Data>();
        }
        ...
        public void save(final ByteBuffer byteBuffer) {
            ...
            if (this.init) {
                ...
                byte b2 = Bits.addFlags(b, 1);
                if (this.doNothing) {
                    b2 = Bits.addFlags(b2, 2);
                }
                byteBuffer.put(this.noiseMainByte);
                byteBuffer.put((byte)this.soil);
                byteBuffer.put(this.magicNumByte);
                final int size = this.regions.size();
                if (size == 1) {
                    b2 = Bits.addFlags(b2, 4);
                }
                else if (size == 2) {
                    b2 = Bits.addFlags(b2, 8);
                }
                else if (size == 3) {
                    b2 = Bits.addFlags(b2, 16);
                }
                else if (size == 4) {
                    b2 = Bits.addFlags(b2, 32);
                }
                else if (size > 4) {
                    b2 = Bits.addFlags(b2, 64);
                    byteBuffer.put((byte)this.regions.size());
                }
                for (int i = 0; i < this.regions.size(); ++i) {
                    this.regions.get(i).save(byteBuffer);
                }
        ...
        public void load(final ByteBuffer byteBuffer, final int n) {
            ...
            this.init = Bits.hasFlags(value, 1);
            if (this.init) {
                this.doNothing = Bits.hasFlags(value, 2);
                this.noiseMainByte = byteBuffer.get();
    ...
    public static class Chunk
    {
        ...
        public void set(final IsoChunk isoChunk) {
            this.x = isoChunk.wx;
            this.y = isoChunk.wy;
        }
